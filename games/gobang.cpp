...
Player Gobang::get_winner(int x, int y) {
    int l = 0;
    int r = 0;
    while (x - l - 1 >= 0 && state.board[x][y] == state.board[x - l - 1][y])
        ++l;
    while (x + r + 1 < GOBANG_BOARD_SIZE &&
           state.board[x][y] == state.board[x + r + 1][y])
        ++r;
    if (l + 1 + r >= WIN_LINE) return state.board[x][y];
    l = 0;
    r = 0;
    while (y - l - 1 >= 0 && state.board[x][y] == state.board[x][y - l - 1])
        ++l;
    while (y + r + 1 < GOBANG_BOARD_SIZE &&
           state.board[x][y] == state.board[x][y + r + 1])
        ++r;
    if (l + 1 + r >= WIN_LINE) return state.board[x][y];
    l = 0;
    r = 0;
    while (x - l - 1 >= 0 && y - l - 1 >= 0 &&
           state.board[x][y] == state.board[x - l - 1][y - l - 1])
        ++l;
    while (x + r + 1 < GOBANG_BOARD_SIZE && y + r + 1 < GOBANG_BOARD_SIZE &&
           state.board[x][y] == state.board[x + r + 1][y + r + 1])
        ++r;
    if (l + 1 + r >= WIN_LINE) return state.board[x][y];
    l = 0;
    r = 0;
    while (x - l - 1 >= 0 && y + l + 1 < GOBANG_BOARD_SIZE &&
           state.board[x][y] == state.board[x - l - 1][y + l + 1])
        ++l;
    while (x + r + 1 < GOBANG_BOARD_SIZE && y - r - 1 >= 0 &&
           state.board[x][y] == state.board[x + r + 1][y - r - 1])
        ++r;
    if (l + 1 + r >= WIN_LINE) return state.board[x][y];
    return None;
